...
void Algorithms::DFS(const Graph &myGraph, size_t vertex, std ::vector<bool> &visited)
{                           // want to implement the DFS algo
    visited[vertex] = true; // will mark the current vertex as visited in the same index
    for (size_t i = 0; i < myGraph.getNumVertices(); i++)
    {
        if (myGraph.isEdge(vertex, i) && !visited[i])
        {                             // check if the conditions for running DFS exist
...
bool Algorithms::isConnected(const Graph &myGraph)
{
    ...
    { // loop over all the vertices in the graph
        if (!visited[i])
        { // check if a vertex has been visited if it had the graph isn't connected to that vertex
            return false;
        }
    }
    return true; // if we visited all vertices then all are connected(also the graph is connected)
}
bool Algorithms ::containsCircleHelper(const Graph &myGraph, size_t ver, size_t parent, std ::vector<bool> &visited, std::vector<int> &verInCircle)
{
    visited[ver] = true;
    verInCircle.push_back(ver);

    for (size_t u = 0; u < myGraph.getNumVertices(); ++u)
    {
        if (myGraph.getAdjMat()[ver][u])
        {
            if (!visited[u] ){
                 return containsCircleHelper(myGraph, u, ver, visited, verInCircle);
            }
            else if (u != parent)
            {
                cout << "Cycle found: ";
                for (size_t j = 0; j < verInCircle.size(); ++j)
                {

                    cout << verInCircle[j];
                    if (j < verInCircle.size() - 1)
                    {
                        cout << " -> ";
                    }
                }
...
bool Algorithms::isContainsCycle(const Graph &myGraph)
{
    ...
    // vector<int>parent(totalVer,false);
    vector<int> verInCircle;
    for (size_t i = 0; i < totalVer; ++i)
    {
        if (!visited[i])
        {
            if (containsCircleHelper(myGraph, i, SIZE_MAX, visited, verInCircle))
            {
                return true;
            }
        }
